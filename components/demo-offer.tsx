...
export function DemoOffer({ language, state, remaining, onConnect, busy }: {
  language: Language; state: DemoState; remaining: number; onConnect: () => void; busy: boolean;
}) {
  const t = messages[language];
  const descriptions = { loading: t.demoLoading, disabled: t.demoDisabled, available: t.demoAvailable, used: t.demoUsed, limit: t.demoLimit, unavailable: t.demoUnavailable };
  return <aside className="demo-offer" aria-label={t.demoTitle}>
    <div><strong>{t.demoTitle}</strong><p role="status">{descriptions[state]}</p>
      ...
      {state === "available" && <small>{t.demoAttemptNote}</small>}
    </div>
